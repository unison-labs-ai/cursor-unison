...
function formatContext(recentDocs, searchResults) {
  const hasRecent = recentDocs.length > 0;
  const hasSearch = searchResults.length > 0;
  if (!hasRecent && !hasSearch)
    return "";
  const sections = ["[UNISON BRAIN CONTEXT]"];
  if (hasSearch) {
    sections.push(`
Relevant Knowledge:`, ...searchResults.map((r) => {
...
async function main() {
  ...
  const workspaceHint = input.workspace_roots[0] ? input.workspace_roots[0].split("/").filter(Boolean).pop() ?? "" : "";
  const [recentResult, searchResult] = await Promise.allSettled([
    client.list({ prefix: "/private/notes/", limit: config.maxProjectDocs }),
    workspaceHint ? client.search(workspaceHint, { limit: config.maxResults }) : Promise.resolve([])
  ]);
